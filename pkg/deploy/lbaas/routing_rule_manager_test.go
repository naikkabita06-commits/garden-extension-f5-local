...
func (s *stubRoutingRuleClient) CreateRoutingRule(_ context.Context, _, _ string, spec RoutingRuleSpec) (RoutingRuleResource, error) {
	...
	return RoutingRuleResource{ID: "rule-1", Host: spec.Host, Path: spec.Path, PoolID: spec.PoolID}, nil
}
...
func TestRoutingRuleManagerEnsureCreatesMissingRules(t *testing.T) {
	client := &stubRoutingRuleClient{rules: []RoutingRuleResource{{ID: "rule-existing", Host: "example.com", Path: "/", PoolID: "pool-1"}}}
	rules, changed, err := NewRoutingRuleManager(client).Ensure(context.Background(), "lb-1", "vs-1", []RoutingRuleSpec{{Host: "example.com", Path: "/", PoolID: "pool-1"}, {Host: "api.example.com", Path: "/v1", PoolID: "pool-2"}})
	if err != nil {
