...
func (d *Deployer) CleanupGraph(ctx context.Context, req GraphCleanupRequest) error {
	...
	for _, key := range sortedObservedKeys(current.Graph.RoutingRules) {
		rule := current.Graph.RoutingRules[key]
	...
	for _, key := range sortedObservedKeys(current.Graph.Members) {
		member := current.Graph.Members[key]
	...
	for _, key := range sortedObservedKeys(current.Graph.Monitors) {
		monitor := current.Graph.Monitors[key]
		vsName, poolName, ok := graphPoolParts(key)
		if !ok {
	...
	for _, key := range sortedObservedKeys(current.Graph.Pools) {
		pool := current.Graph.Pools[key]
	...
	for _, key := range sortedObservedKeys(current.Graph.VirtualServers) {
		vs := current.Graph.VirtualServers[key]
		if err := d.client.DeleteVirtualServer(ctx, lbID, vs.ExternalID); err != nil {
			return fmt.Errorf("deleting virtual server %s: %w", key, err)
		}
	}
	if req.DeleteVIP {
		for _, key := range sortedObservedKeys(current.Graph.VIPs) {
			vip := current.Graph.VIPs[key]
...
func sortedObservedKeys(resources map[string]model.ObservedResource) []string {
